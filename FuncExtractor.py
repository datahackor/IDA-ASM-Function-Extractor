        ...
        # ea is in the middle of the function tree
                ...
                for next_ea in func_relations[func_ea][direction]:
                    ...
                    # next ea in a function.
                        ...
                        # found a new callee and caller, process next callee and caller
                            ...
                            _append_result(result, next_ea, next_func_ea, next_func_type, dref_off_ea, direction, func_relations, append_exceeded=False)
                            if debug: dbg_print("entering a recursive call 1", nrecursive)
                            ...
                            for r in trace_func_calls(func_relations, next_ea, next_func_ea, target_ea, direction, vtbl_refs, result, force_trace, max_recursive, nrecursive+1, skip_lib, skip_api, filtered_nodes, finished, finished_in_path, finished_nodes_in_path, debug, dbg_print_func):
                                yield r
                            _pop_result(result, next_ea, next_func_ea, next_func_type, dref_off_ea, direction, append_exceeded=False)
                            if debug: dbg_print("leaving a recursive call 1", nrecursive)
                        # found a recursive call
                        else:
                            if debug: dbg_print("detecting a recursive call 1")
                            if debug: dbg_print("next_ea:", hex(next_ea).rstrip("L"), "next_func_ea:", hex(next_func_ea).rstrip("L"), "dref_off_ea:", hex(dref_off_ea).rstrip("L"), "next_func_type:", next_func_type, "ea:", hex(ea).rstrip("L"), "func_ea:", hex(func_ea).rstrip("L"))
                        ...
                        if (next_func_ea, next_func_ea, next_func_type) not in result:
                            _append_result(result, next_ea, next_func_ea, next_func_type, dref_off_ea, direction, func_relations, append_exceeded=False)
                            if debug: dbg_print("entering a recursive call 2", nrecursive)
                            # we need callee (nexgt_func_ea) as the second argument. do not change it to next_ea that is a caller.
                            for r in trace_func_calls(func_relations, next_ea, next_func_ea, target_ea, direction, vtbl_refs, result, force_trace, max_recursive, nrecursive+1, skip_lib, skip_api, filtered_nodes, finished, finished_in_path, finished_nodes_in_path, debug, dbg_print_func):
                                yield r
                            _pop_result(result, next_ea, next_func_ea, next_func_type, dref_off_ea, direction, append_exceeded=False)
                            if debug: dbg_print("leaving a recursive call 2", nrecursive)
            ...
            for next_ea, next_func_ea, dref_off_ea, func_type in drefs_wrapper(drefs, func_relations, direction, vtbl_refs, debug=debug, dbg_print_func=dbg_print_func):
                ...
                if func_ea == ida_idaapi.BADADDR:
                    ...
                    if recurse_flag:
                        if debug: dbg_print("entering a recursive call 3", nrecursive)
                        for r in trace_func_calls(func_relations, next_ea, next_func_ea, target_ea, direction, vtbl_refs, result, force_trace, max_recursive, nrecursive+1, skip_lib, skip_api, filtered_nodes, finished, finished_in_path, finished_nodes_in_path, debug, dbg_print_func):
                            yield r
                ...
                # for offset?
                    ...
                    if debug: dbg_print("entering a recursive call 4", nrecursive)
                    for r in trace_func_calls(func_relations, next_ea, next_func_ea, target_ea, direction, vtbl_refs, result, force_trace, max_recursive, nrecursive+1, skip_lib, skip_api, filtered_nodes, finished, finished_in_path, finished_nodes_in_path, debug, dbg_print_func):
                        yield r
                    result.pop(-1)
...
def get_line_number(phrase, file_name):
    try:
        ...
        print("get_line_number failed! ", e, phrase , i)
    except :
        print("get_line_number except! ", phrase , i)	
        sys.exit()
    finally :
        return i
...
def export_variables(phrase, lstfile_name, export_filename):
    try:
        with open(lstfile_name) as f:
            for i, line in enumerate(f, 1):
                idx = line.find(phrase)
                if idx != -1:
                    #print ("%s" % (line)[20:], file=export_filename)
                    export_filename.write((line)[idx+9:])
    except Exception as e:
        print("export_variables failed! ", e, phrase , i)	
    except :
        print("export_variables except! ", phrase , i)	
        sys.exit()		
    finally:
        return idx
# end of export_variables

#------------------------------------------------------------------------------------------------------------------------

def export_function_asm(startline, endline, lstfile_name, export_filename):
    try:
        with open(lstfile_name) as f:
            for i, line in enumerate(f, 1):
                if i >= startline  and i <= endline:
                    #print ("%s" % (line)[20:], file=export_filename)
                    export_filename.write((line)[14:])
    except Exception as e:
        print("export_function_asm failed! ", e, startline ,endline)	
        sys.exit()
    except :
        print("export_function_asm except! ", startline ,endline)	
        sys.exit()
		
# end of export_function_asm
...
def main():
    ...
    varCount = 0
    outasm_filename = open(idautils.GetIdbDir() + "\\" + func_name + ".asm" ,"a+")
    asmlst_filename = basename + ".lst"  
    while ea != ida_idaapi.BADADDR:
        ...
        funclist.append(ea)
        print("Func Type & start ea : end ea", func_type, hex(ea).rstrip("L"), hex(f.end_ea).rstrip("L"))
        ...
        for x in children:
            if children[x][1] <= 8 :
                print("Children :" ,hex(x).rstrip("L"), hex(children[x][0]).rstrip("L"), children[x][1], children[x][2], children[x][3])
            if children[x][1] == 1 and children[x][0] != ida_idaapi.BADADDR and callees.__contains__(children[x][0]) == False:
                callees[children[x][0]] = x
                if funclist.__contains__(children[x][0]) == False:
                    funclist.append(children[x][0])
    ...
    # export func
    funclist.sort();
    print("func count : ", len(funclist))
    for fea in funclist:
        f, _ = get_func_bbs(fea)
        if f:
            function_start_phrase = ":" + hex(f.start_ea)[2:].upper()
            function_end_phrase = ":" + hex(f.end_ea)[2:].upper() 
            func_asm_start_address = get_line_number(function_start_phrase, asmlst_filename)
            func_asm_end_address = get_line_number(function_end_phrase, asmlst_filename)
            export_function_asm(func_asm_start_address,func_asm_end_address,asmlst_filename,outasm_filename)

    # export variables
    varlist.sort();   
    print(varCount, len(varlist))
    #sys.exit(0)
    for var in varlist:
        print("export_variables : ", hex(var) )
        if( var > ida_nalt.get_imagebase() ):
            export_variables(":" + hex(var)[2:].upper(), asmlst_filename,outasm_filename)

    print("Extract Function END !")
    for x in vtbl:
        print("Vftable: %x" % x)
